...
import { dismissDb, dbs, monitorReady, startDb, stopDb } from '../stores/monitor'
...
<template>
  <div class="h-full">
    <ViewLoading v-if="!monitorReady" label="正在扫描本机数据库实例…" :cards="2" />
          ...
          >
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-2">
                ...
                <span
                  v-if="row.version"
                  class="rounded-[5px] border border-line px-1.5 font-mono text-[10px] uppercase text-ink-soft dark:border-coal-line dark:text-chalk-soft"
                >
                  v{{ row.version }}
                </span>
              </div>
      ...
      <section v-else class="card grid place-items-center gap-3 border-dashed py-16 text-center">
        ...
        <div>
          ...
          <p class="mt-1 font-mono text-xs text-ink-soft dark:text-chalk-soft">
            启动 MySQL / Redis / PostgreSQL 等后会自动出现在这里
          </p>
        </div>
      </section>
    </div>
