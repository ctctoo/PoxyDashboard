...
import { containers, monitorReady, startContainer, stopContainer } from '../stores/monitor'
...
function statusColor(s: ContainerState): string {
  return s === 'running'
    ...
    : s === 'stopping' || s === 'starting'
      ? 'text-warn'
      : 'text-ink-soft/60 dark:text-chalk-soft/60'
}
...
function dotClass(s: ContainerState): string {
  return s === 'running'
    ...
    : s === 'stopping' || s === 'starting'
      ? 'bg-warn'
      : 'bg-ink-soft/50 dark:bg-chalk-soft/50'
}
...
<template>
  <div class="h-full">
    <ViewLoading v-if="!monitorReady" label="正在扫描本机 Docker 容器…" :cards="2" />
    <div v-else class="scroll-slim h-full space-y-6 overflow-auto pb-4 pr-1">
      ...
      <header class="flex flex-wrap items-center gap-2">
        ...
        <span
          v-if="stoppedCount"
          class="flex items-center gap-2 rounded-[6px] border border-ink/10 bg-ink/5 px-3 py-1.5 font-mono text-xs text-ink-soft dark:border-white/10 dark:bg-white/5 dark:text-chalk-soft"
        >
      ...
      <section v-if="containers.length" class="card overflow-hidden">
        <header
          class="flex items-center justify-between border-b border-line px-4 py-3 dark:border-coal-line"
        >
          ...
          <span class="font-mono text-[11px] text-ink-soft dark:text-chalk-soft"
            >点击端口可打开 · 运行中优先排序</span
          >
        </header>
          ...
          >
            ...
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-2">
                ...
                <span
                  class="rounded-[5px] border border-line px-1.5 font-mono text-[10px] text-ink-soft dark:border-coal-line dark:text-chalk-soft"
                >
                  {{ row.image }}
                </span>
              </div>
              <div
                class="truncate font-mono text-[11px] text-ink-soft dark:text-chalk-soft"
                :title="row.statusText ?? row.portMap"
              >
                {{ row.statusText ?? '—' }}{{ row.portMap ? ` · ${row.portMap}` : '' }}
              </div>
            </div>
            <span
              class="flex items-center gap-1.5 font-mono text-xs font-medium"
              :class="statusColor(row.status)"
            >
              <span class="h-1.5 w-1.5 rounded-full" :class="dotClass(row.status)" />
              {{ statusLabel(row.status) }}
            </span>
            <span v-if="row.ports.length" class="flex items-center gap-1">
              <button
