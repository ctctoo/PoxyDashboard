...
const props = defineProps<{ kind: AppKind; view: 'grid' | 'list' }>()
  ...
  <section>
    ...
    <template v-if="!sort.active">
      <draggable
        ...
        :animation="150"
        class="gap-2"
        :class="props.view === 'list' ? 'flex flex-col' : 'grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))]'"
        @end="onDragEnd"
      >
    ...
    <div v-else class="gap-2" :class="props.view === 'list' ? 'flex flex-col' : 'grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))]'">
      <div
      ...
      >
        <AppCard :entry="e" :layout="props.view" @diagnose="(id: string) => emit('diagnose', id)" />
      </div>
    </div>
